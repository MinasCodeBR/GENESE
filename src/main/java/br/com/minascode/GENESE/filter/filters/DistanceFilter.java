...
public class DistanceFilter<T extends ConfigurableCombination> implements Filter<T> {

    private final RemovalCriteria<T> removalCriteria;
    private final LotteryGameType gameType;

    public DistanceFilter(RemovalCriteria<T> removalCriteria, LotteryGameType gameType) {
    ...
    public List<T> customFilter(List<T> combinations) {
        ...
        if (distancesToRemove != null) {
            for (Integer distanceToRemove : distancesToRemove) {
                combinations.removeAll(distance.getIncidenceList(distanceToRemove));
            }
        }
